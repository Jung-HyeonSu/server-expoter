...
## 역할 설명
3-channel JSON 출력 스키마 관리. `sections.yml` (11 섹션) + `field_dictionary.yml` + `baseline_v1/` (벤더별 회귀 기준선) + `callback_plugins/json_only.py` (출력 envelope) 일관성 보장. fragment 정규화 → 표준 JSON 조립 파이프라인 (build_*.yml 5종).

## 주요 영역
- `schema/sections.yml` — 11 섹션 정의 (system, hardware, bmc, cpu, memory, storage, network, firmware, users, power, thermal)
- `schema/field_dictionary.yml` — 필드 사전 (런타임 미사용, 테스트·CI·훅 전용)
