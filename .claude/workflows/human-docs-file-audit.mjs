export const meta = {
  ...
  phases: [
    { title: 'Converge', detail: 'verify applied fixes are clean + deep code-truth on docs/01-04,09,16,19,21 (not covered before)' },
...
- ROUND G/G2 fix: cross-channel regression counts. docs/23 bullet command now points to tests/regression/test_cross_channel_consistency.py = 120 (9 baseline × 13 + 3 coverage); the DIRECTORY command pytest tests/regression/ = 158 (120 + HBA/IB 27 + vendor-display 7 + csus-mock 4) — labeled correctly in docs/23 line 271 and docs/14 line 324. VERIFIED by running pytest --collect-only on both scopes. Do NOT re-flag these counts.
- ROUND I fix: docs/09:552 FAQ — diagnosis 공통 고정 필드 5→6 (added failure_reason; diagnosis_mapper.py returns reachable/port_open/protocol_supported/auth_success/failure_stage/failure_reason + details). Do NOT re-flag.
- ROUND H fix (owner-approved): Cisco verification status. docs/10 검증상태 table + REQUIREMENTS §3-1 now say "M4 partial lab 검증 (UCS C220 M4 / CIMC 4.1(2g))" — consistent with docs/13:484 "partial (M4 lab tested)", docs/22 M4=OK, and the real evidence file (tests/evidence/2026-04-29-cisco-redfish-critical-review.md). The previous "실장비 미검증" was the imprecise outlier. Do NOT re-flag Cisco lab status. CLAUDE.md "3대 Dell/HPE/Lenovo" is the FULL-Round set (Cisco M4 is partial) — not a contradiction, and CLAUDE.md is harness anyway.
- KNOWN-HISTORICAL / OUT-OF-SCOPE (do NOT report — already ruled out): docs/13 §11/§12 "(14개 YAML)" frozen 2026-03-18 snapshot; docs/19 "107" dated ticket counts; CLAUDE.md "약 3,867줄" harness; tests/regression/conftest.py docstring "8 baseline JSONs" is a CODE comment handled by a separate comment-audit; docs/23 -vvv "28 candidates" synthetic example.
`
