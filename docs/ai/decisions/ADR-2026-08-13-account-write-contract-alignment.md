...
증명했다. 그 위에서 9 Vendor 공식 조사를 수행한 결과, 코드가 **여전히 조사 결과와 충돌하는
지점**이 남아 있었다. 크게 셋이다.
...
금지**했다(05 §19, 06 §17, 07 §17, 08 §17, 09 §19). 즉 "근거가 없으니 여러 번 시도한다" 는
정반대였다. 근거가 없을수록 한 번만 써야 한다.

**(b) 한 장비의 실측이 세대 전체로 번져 있었다.** `hpe_ilo5plus` 하나가 iLO5/6/7 전 Firmware 를
`evidence='proven'` 으로 덮고 있었는데, 실제로 재현한 것은 **iLO6 v1.73 한 버전**이다. HPE
Advisory `a00159600en_us` 는 iLO6 1.73/1.74 + iLO7 1.19/1.20 을 영향 버전으로, iLO6 1.75+ /
iLO7 1.21+ 를 **해결 버전**으로 명시한다. iLO5 는 어느 쪽 근거도 없다.

**(c) 판정 축이 틀린 곳이 있었다.** 보호 계정을 slot id 문자열(`'HostBootStrap'`)로 맞히려 했는데,
그 값은 collection POST Family 에서는 소비되지 않아 **죽은 값**이었다. 실제로는
...
**표에 없는 Property 의 기본값은 `unverified`** 다. 모르는 속성을 writable 로 가정하지 않는다.
...
예상하지 못한 거부가 오면 **실패로 확정하고 기록**한다. 두 번째 쓰기를 하지 않는다.

### D3. 동작과 근거를 분리한다 (HPE)

Family 를 쪼개지 않는다. 쓰기 동작(Password 단독 PATCH)은 iLO5/6/7 전부 같고, `isolation_basis`
...
기본 `full_body_patch=False` — 실제로 달라진 것만 쓴다. `True` 는 "공통 Property 를 전부
보내라" 가 아니라 **그 Family 에서 writable 로 확인된 상태 Property 를 Password 와 함께 보내야만
하는 예외**다. LIVE/공식 근거가 있는 Family 에만 명시한다.
...
UNVERIFIED Family 는 이 예외를 상속하지 않으며, `True` 여도 read_only/unsupported/unverified
Property 는 실리지 않는다.

### D5. Create URI 를 Accounts 열거 URI 와 분리한다

`accounts_collection` / `account_service_root` / `account_instance` 세 종류. 어느 경우에도
하나가 실패하면 다른 URI 로 갈아타지 않는다. Supermicro 최신 계약(`POST /AccountService`)은
...
현재 어떤 Family 도 Create 에 If-Match 를 요구하지 않는다(테스트로 고정).
...
`reserved_slot_ids` 는 이 판정에 쓰지 않는다. 그것은 "생성할 때 이 슬롯은 고르지 마라" 이지
"그 슬롯의 계정은 고칠 수 없다" 가 아니다 — 섞으면 예약 슬롯에 자리 잡은 표준 계정을 영영
복구하지 못한다.

### D8. 진단은 늘리되 정책은 바꾸지 않는다

Huawei 계정별 Redfish Login Interface, `HTTPBasicAuth` / OEM `AuthMethods`, `policy_conflict`,
계정 잠금 전 검증 중단, 미지원 RoleId — 전부 **읽기·보고 전용**이다. Basic Auth 를 켜지도,
...
**Create / Repair 는 조건이 발생하지 않아 미증명** — 4대 모두 표준 계정이 이미 정상이고,
조건을 만들려면 운영 계정을 지워야 하므로 하지 않았다.

Supermicro / Huawei / Inspur / Fujitsu / Quanta 는 실장비 0대 — **어느 Family 도 PROVEN 으로
올리지 않았다.**
