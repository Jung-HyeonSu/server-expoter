...
전수 감사에서 하네스가 스스로 부풀어 있다는 게 드러났다. 세 가지가 겹쳤다.

**하나. 같은 일을 하는 표면이 이름만 다르게 여러 벌 있었다.** schema 리뷰어 3편은
전부 rule 13 을 근거로 삼고 envelope 필드와 빌더 패턴을 중복 검사하면서, 서로를
"자가 검수 금지" 대상으로 **순환 지목**하고 있었다. A 는 B 에게, B 는 C 에게,
...
**둘. 중복이 오류를 숨겼다.** 표면이 나뉘어 있으니 한 곳을 고쳐도 나머지가
낡은 채 남았다. 실제로 발견한 것 —

| 표면 | 낡은 내용 | 사실 |
|---|---|---|
| `output-schema-reviewer` | "OUTPUT 태스크 **prefix** 식별" | callback 은 **완전일치** 비교다 (`json_only.py:108`). prefix 로 알고 검수하면 `OUTPUT: 결과 출력` 같은 이름을 통과시키고, 그러면 호출자가 빈 응답을 받는다 |
...
**셋. rule 70 R5 가 "남긴다"고 지정한 유형이 실제로는 안 읽히고 있었다.**
R5 는 "다음 작업에서 AI 가 참조하면 도움이 되는가"를 기준으로 제시하는데,
...
유형 표를 면제 목록으로 쓰지 않는다. 유형은 **판정을 도우는 예시**이고,
판정은 언제나 "지금 코드 기준으로 다음 작업에 쓰이는가" 하나다. 그 결과
...
- 검수 순환 해소 — 합친 에이전트는 **다른 축**의 에이전트에게만 위임한다

## 대안 비교 (Considered)

**(A) 그대로 둔다.** 이름이 많으면 고르기 쉽다는 논리. 기각 — 실제로는 반대였다.
같은 일을 하는 이름이 3개면 어느 걸 부를지 판단 비용이 생기고, 셋 중 하나만
고쳐지면서 낡은 둘이 남는다. 위 표의 오류 3건이 그 결과다.
