...
| `diagnosis` 안에 `details` 가 채널마다 다른 키를 가져요 | 의도된 동작. `details` 는 채널별 추가 진단 정보 (os=detected_os, esxi=vsphere_endpoint/auth, redfish=redfish_version/product) 를 담는 free-form dict. 공통 5필드(reachable/port_open/protocol_supported/auth_success/failure_stage)는 항상 고정. |
