...
이 둘이면 "무슨 시스템인지" 는 잡힌다. 그다음은 역할에 따라 갈린다.
...
**숫자는 계약인 것만 적는다.** 섹션 11종, 봉투 13필드, `failure_stage` 6종 같은 건
호출자와의 약속이라 적는다. 어댑터 개수나 테스트 개수처럼 하나만 늘어도 틀리는 수는
적지 않고, 대신 세는 방법을 적어 둔다.

**진술에는 근거가 붙는다.** 코드에서 확인한 것은 `파일:줄`, 장비에서 확인한 것은
검증 기록을 가리킨다.

저장소 밖으로 나가지 않는 AI 협업용 문서는 `docs/ai/` 에 따로 있다. 이 디렉터리의
문서들은 그쪽을 참조하지 않는다 — 배포 브랜치에는 `docs/ai/` 가 없기 때문이다.
