...
`vault/<loc>/redfish/<vendor>.yml` (자격증명) + 내부 벤더 노트{vendor}.md` (도메인 노트) 도 의무 (rule 50 R2 9단계).
...
| 5 (baseline) | SKIP — 내부 후속 작업 목록 "사이트 fixture 캡처" + "baseline 추가" 등재 |
...
python 내부 검증 스크립트    # vendor 경계 (rule 12)
