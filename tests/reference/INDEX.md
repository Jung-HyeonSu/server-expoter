...
> 이 카탈로그는 2026-04-28 수집분에서 멈춰 있다. 그 뒤로 reference 를 새로 뜬 적이
> 없어서 갱신할 내용이 없었다 — 낡은 게 아니라 그 시점이 마지막이다.
> 다음에 reference 를 새로 수집하면 위 표에 행을 추가하고 아래 채널별 항목도 함께 고친다.
> 수치와 디렉터리는 적어 두지 말고 그때그때 센다 (rule 00).
