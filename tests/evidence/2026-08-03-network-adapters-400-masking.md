...
> **[정정 2회]** 잔여 2대의 원인을 두 번 잘못 적었다. 아래가 실측으로 남는 사실이다.
>
> - **1차 오기**: "15.15.08 은 WWN 미노출이라 원래 0" → **반증**. 빌드 #4 에서 8대 전부 `hbas=4`,
>   두 펌웨어 모두 MAC 파생 WWN 노출(`.51` `20:01:d4:ae:52:9e:b4:fb` / `.52` `20:01:90:b1:1c:1f:e2:8e`).
> - **2차 오기**: "NIC 펌웨어가 구분 요인" → **반증**. 빌드 4/5/6 전수 대조:
>
>   | IP | NIC fw | #4 | #5 | #6 |
>   |---|---|---|---|---|
...
>   | **.152** | **16.10.04** | 4 | **4** | 0 |
>
>   `.53` 과 `.52` 는 **같은 펌웨어인데 #5 결과가 갈렸다** → 펌웨어는 구분 요인이 아니다.

**확정 사실**:

- 강등 fix(#5)는 8대 중 6대에서 효과가 있었고, `.52` / `.152` 두 대에서만 실패했다.
- 실패한 2대의 `storage.hbas[].port_id` 는 포트 id 가 아니라 **NDF id**(`NIC.Integrated.1-1-1`) —
