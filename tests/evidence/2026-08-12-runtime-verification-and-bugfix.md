...
`merge_fragment.yml` 이 깊이 2 에서 dict 를 통째로 교체하기 때문에 나중에 실행되는
...
`normalize_system.yml` 과 동일한 원본을 이어받는다. `collect_disks` 가 먼저 실행되므로
...
`{{ playbook_dir }}` 는 실행된 playbook 의 디렉터리인 `<repo>/redfish-gather` 로 해석된다.
그 아래에는 `common/` 이 없다.
...
해당 벤더의 **OEM fragment 가 한 번도 병합되지 않았다.**

### 실제 영향 — fragment 미병합보다 크다 (실측)

문제의 `include_tasks` 는 벤더 task 파일의 최상위에 있고 그 파일을 부르는
...
rescue 가 graceful 이라 `status` 는 success/partial 로 남는다. **아무도 눈치채지
...
주입 실험으로 가드 유효성을 확인했다. 두 버그를 각각 되돌리면 `exit=1` 로 검출된다.
...
inventory 자격으로 수집이 진행되는데 그 경우 `_os_auth_ok` 는 false 로 남지만 데이터는
실제로 수집된다. 두 번째 항이 없으면 envelope 이
...
> 결함이었다. 필터에 `grep -viE "password|secret|token"` 을 걸어 뒀는데 Dell envelope 이
...
- `auth_success=null` 은 인증을 시도하지 못했으므로 false 가 아님
- 요청 1 → envelope 1
...
`_rf_auth_observations` 중 `role == 'primary'` 이면서 `status == 401` 정확 일치가
...
- Recovery 자격으로 진입해 예정 Action(`password_sync`, slot 3)만 보고
- `dryrun: true` 이므로 **실제 Account Write 0건**
- 그럼에도 `status=success`, `errors=[]` 이라 P0-7 (성공한 fallback 을 오류/partial 로
  만들지 않는다) 실장비 검증
...
- Jenkins 가 받는 SHA 는 Job 의 SCM 설정에 달려 있으며 **push 성공만으로 Jenkins
...
`errors.message` 개선 및 Runtime 검증 Phase는 여기서 종료한다.
...
> pytest PASS 는 본 Phase 커밋(`be97e407`) 트리를 격리 추출해 실행한 결과다
...
`main -> production` 승격 대상 순수 코드 32 파일 중 31 파일이 Resolver 소유이고
본 Phase 소유는 `docs/reference/decision-log.md` 1 파일뿐이다.

`scripts/ai/promote_to_production.sh` 는 `origin/production ↔ main` 의 순수 코드 차이를
전량 승격한다. 파일 선택 인자가 없고 저장소에 다른 공식 선택 승격 경로도 없다.
`docs/reference/decision-log.md` 하나를 넣으려면 Resolver 코드 31 파일이 함께 올라간다.

경우 2 로 판정하고 `docs/reference/decision-log.md` 는 계속 대기시킨다.
`git merge main -> production` / force push / history rewrite 는 사용하지 않았다.

본 Phase 의 게더링 코드 변경은 이미 production(`b574841e`)에 전부 반영돼 있다
(ESXi `listening_ports`, Redfish OEM include 6건, 회귀 테스트 2건).
대기 중인 것은 `docs/reference/decision-log.md` 문서 교정 1건뿐이며 운영 동작에 영향이 없다.

Resolver 의 완료·검증·commit·production 승격 판단은 해당 작업 세션 소관이다.
